...
func (src *Source) Generate(w io.Writer) error {
	...
	funcMap := template.FuncMap{
		...
		"newlazydll": func(dll string) string {
			arg := "\"" + dll + ".dll\""
			return fmt.Sprintf("%vNewLazyDLL(%v)", syscalldot(), arg)
		},
	}
...
{{define "dlls"}}{{range .DLLs}}	{{.}} = {{newlazydll .}}
{{end}}{{end}}

{{define "funcnames"}}{{range .DLLFuncNames}}	__{{.DLLFuncName}} = {{.DLLName}}.NewProc("{{.DLLFuncName}}")
{{end}}{{end}}
...
{{define "syscall"}}{{.Rets.SetReturnValuesCode}}{{.Syscall}}(__{{.DLLFuncName}}.Addr(), {{.ParamCount}}, {{.SyscallParamList}}){{end}}
